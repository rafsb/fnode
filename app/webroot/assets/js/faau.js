...
const
    ...
    ROW = (e, s) => {
        const x = DIV("-row", s);
        typeof e == "string" ? x.html(e) : x.app(e);
        return x
    },
        ...
        arr.each(n => {
            switch (n) {
                case "Y":
                    ...
                    break;
                case "y":
                    format = format.replace(n, (d.getYear() - 100).fill("0", 2));
                    break;
                case "m":
                    format = format.replace(n, (d.getMonth() + 1).fill("0", 2));
                    break;
                case "d":
                    format = format.replace(n, d.getDate().fill("0", 2));
                    break;
                case "h":
                    format = format.replace(n, d.getHours().fill("0", 2));
                    break;
                case "i":
                    format = format.replace(n, d.getMinutes().fill("0", 2));
                    break;
                case "s":
                    format = format.replace(n, d.getSeconds().fill("0", 2));
                    break;
                case "k":
                    format = format.replace(n, d.getMilliseconds().fill("0", 3));
                    break;
                case "t":
...
binds(HTMLInputElement.prototype, {
    ...
    up: function(name, path, callback = null) {
        if (!this.getAttribute('type').toLowerCase() == 'file') return console.trace('input.up only available for input[type="file"]');
        const
            ctnr = this.uid(),
            form = new FormData(),
            count = 0;
        ...
        // this.files.forEach(file => form.append(`file.${count++}`, file))
        ...
        fetch('/upload', {
            ...
            body: form,
            mode: 'no-cors'
        }).then(x => x.json()).then(x => callback(new CallResponse('/up', body, method = "POST", header = {}, x)))
    }
});
        ...
        while (i--) {
            if (tmp[i]) {
                if (!this.classList.contains(tmp[i]))
                    this.classList.add(tmp[i]);
                else this.classList.remove(tmp[i]);
            }
        }
        return this;
    },
...
class FAAU {
    ...
    notify(n, c = null) {
        ...
        toast.onclick = function() {
            ...
            this.desappear(ANIMATION_LENGTH / 2, true);
        };
    ...
    hashit(o) {
        if (typeof o == "object" || typeof o == "array") o = JSON.stringify(o);
        return {
            hash: btoa(o)
        }
    }
...
binds(window, {
    maxis: { x: 0, y: 0 },
    $: function(wrapper = null, context) { return [].slice.call((context || document).querySelectorAll(wrapper)) },
