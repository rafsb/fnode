...
global.LOG = (tx, persist=false, filename = 'default.log') => {
    const m = FDate.as('<Y-m-d_h:i:s> ') + (typeof tx == 'object' ? JSON.stringify(tx) : tx) ;;
    persist ? IO.log(m, typeof pesist == 'string'? persist : filename, EIO.APPEND) : console.log(m)
}
