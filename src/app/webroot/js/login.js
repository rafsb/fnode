(function(app, args){
    ...
    const
    d = DIV('wrap relative').app([
        TAG('form', 'col-2 centered', { background:`{{background}}`, padding:`1em`, borderRadius:`.5em`, boxShadow:`0 0 .5em {{dark3}}` }).attr({ action:'javascript:void(0)' }).app(
            TAG('header', 'row', { marginBottom: `2em` }).app(
                IMG('img/logo.png', 'left', { height:'3em', width:'3em', borderRadius:'50%', boxShadow:'0 0 1em {{dark2}}' })
            ).app(
                SPAN('LOGIN', 'col-6 right content-right', { color:'white', borderRadius:'0 1.5em 1.5em 0', padding:`1em 1.5em`, backgroundImage: 'linear-gradient(to left, {{belize_hole}}44, transparent)' })
            )
        ,
            DIV('row').app(
    ...
    $('#app').empty().css({ background: "{{foreground}}", color:"{{silver}}" }).app(d)
