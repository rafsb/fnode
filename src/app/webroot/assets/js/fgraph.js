...
class FGraph {
    ...
    __DrawLines(o){
        ...
        set.each((item, i) => {
            ...
            o.node.app(SPATH(d.join(" "), "--line avoid-cursor", {
    ...
    __DrawBars(o, set){
        ...
        set.extract((item, i) => {
            ...
            serie.each((n, j) => {
                ...
                ;;
                o.node.app(SVG("rect", "avoid-cursor", { width: w, height: h - y, x: x - w / 2, y: y }, { fill: color }))
            })
        })
    }
    ...
    __DrawGBars(o){
        ...
        serie.each((n, i) => {
            ...
            , x =  xpace / 2 + i * xpace
            ;;
            o.node.app(SVG("rect", "avoid-cursor -bar", { width: w, height: h - y, x: x - w / 2, y: y }, { fill: color }))
        })
    }
    ...
    __DrawSBars(o){
        ...
        serie.each((n, i) => {
            ...
            n.each((e, j) => {
                ...
                ;;
                plot[j] += (e / ymax);
                o.node.pre(SVG("rect", "avoid-cursor", { width: w, height: h, x: x - w / 2, y: h - y }, { fill: color }))
            })
        })
    }
                ...
                o.node.app(
                    SVG("rect", "-hint-plate pointer --tooltip", { width: o.rects.width, height: o.rects.height, x: 0, y: 0 }, {
                        ...
                        `<b class='row flex' style='color:${set.color || PRISM.array()[i % PRISM.array().length]}'>\
                            ...
                            <span class='col-4 content-right' style='marginleft:1em'>${set.plot.calc(SUM).nerdify()}</span>\
                        </b>` : null
                    }).join('') })
                )
            } else {
