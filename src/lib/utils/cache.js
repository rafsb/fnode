...
module.exports = class fCache {
    ...
    set(name, data, expires = 1000 * 60 * 60 * 24) {
        const now = date.time() ;;
    ...
    all(){
        ...
        return Object.keys(all).map(k => {
            let res = all[k] ;;
            res.id = k
            if(date.time() > res.expires) {
                this.db.destroy(name, null, true)
                res = null
            }
            return res
        }).filter(i => i)
    ...
    static set(name, data, time) {
        if(!(name && data)) return false
        const c = fCache.cast(name) ;;
        return c.set(name, data, time)
    }
