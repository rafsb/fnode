const
    connectionString = "", // required to work
    ...
    static push(database, collection, record, fn) {
        mongoClient.connect(connectionString, function(e, db) {
            ...
            dbase.collection(collection, (e, coll) => {
                ...
                coll.insertMany(Array.isArray(record) ? record : [record], (e, res) => {
                        fn && fn({ e: e, data: res });
                        db.close()
                    })
    ...
    static refresh(database, collection, ref, record, fn) {
        mongoClient.connect(connectionString, function(e, db) {
            ...
            const
                dbase = db.db(database);;
            dbase.collection(collection, (e, coll) => {
                coll.updateMany(ref, { $set: record }, (e, res) => {
                    fn && fn({ e: e, data: res.result.nModified });
                    db.close()
                })
            })
        })
