...
module.exports = class IO {
    static root(path=null){
        if(path[0] == "/" || path.match(/\b[a-zA-Z]{1}:.*/gi)) return path;
        return ROOT + path
    }
    ...
    static jout(path){
        return FObject.instance(JSON.parse(IO.read(path)));
    }   

    static log(content, f="debug.log", mode = EIO.APPEND)
    {
        var tmp = [], offset ;
        f = "var/logs/" + f;
    ...
    {
        if(folder===null || !fs.existsSync(IO.root(folder))) return [];
        var 
    ...
    static folders(path)
    {
        ...
        ;;
        if(tmp.length) tmp.forEach(f => { if(fs.lstatSync(folder + "/" + f).isDirectory()) arr.push(f) });
        return arr;
    }
